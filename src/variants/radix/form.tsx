 ...
 *
 * Unlike every other file in this directory, this one uses no Radix package -- and that is
 * a deliberate choice rather than an omission. @radix-ui/react-form exists, but it is built
 * on native constraint validation with `match` predicates, it has no Description part, and
 * its Message API is shaped nothing like `validate` returning a string. Adopting it would
 * have changed the public API, which is the one thing a variant may not do: the same story
 * and the same block have to compile against all three trees. shadcn's own form component
 * takes this same route for the same reason.
 *
